...
def print_updated_progress(index, graphs_skipped, start_time):
    if index % CHUNK_SIZE == 0:
        graphs_processed = index - graphs_skipped
        graphs_remaining = GRAPHS_WITH_N_10 - index
        time_remaining = estimate_time_remaining(start_time, graphs_processed, graphs_remaining)
        print(f'{index:,} total graphs processed, {graphs_remaining:,} remaining. Time remaining: {time_remaining}', end='\r')
...
def skip_graphs_until(graphs, index):
    for curr_index, _ in enumerate(graphs, start = 1):
        if curr_index % 1000 == 0:
            print(f'Skipped {curr_index:,}/{index:,} graphs...', end='\r')
        if curr_index == index:
            print()
            break
...
def skip_processed_graphs(graphs):
    ...
    if last_processed > 0:
        print(f'Skipping first {last_processed:,} graphs...')
...
def fill_csvs_paralelly(n=10):
    ...
    with mp.Pool() as pool:
        ...
        for index, (graph6, thinness, is_minimal) in enumerate(process_map, start=graphs_skipped + 1):
            ...
            print_updated_progress(index, graphs_skipped, start_time)