    ...
    def test(self):
        for vertices_per_side in range(3, 20):
            with self.subTest(vertices_per_side=vertices_per_side):
                solution: ConsistentSolution = thinness_of_lean_spider_graph(vertices_per_side)
                ...
                self.assertEqual(solution.thinness, 1+floor(vertices_per_side / 3))
                self.assertTrue(verify_solution(build_lean_spider_graph(vertices_per_side), solution))
